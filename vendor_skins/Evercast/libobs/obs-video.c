...
static struct obs_source_frame* find_hotwire_frame()
{
	struct obs_core_data *data = &obs->data;
	struct obs_source *source;
	struct obs_source_frame *result = NULL;
	...
	while (source) {
		...
		if (cur_source) {
			result = get_hotwire_frame(cur_source);
			if (NULL != result) {
				break;
			}
		}
...
static void output_hotwire_frame(const struct obs_source_frame *hotwire_frame)
{
	if (NULL == hotwire_frame) {
		return;
	}
	...
	memset(&frame, 0, sizeof(struct video_data));
	frame.timestamp = hotwire_frame->timestamp;
	for (int i = 0; i < MAX_AV_PLANES; i++) {
		frame.linesize[i] = hotwire_frame->linesize[i];
		frame.data[i] = hotwire_frame->data[i];
	}

	memset(&info, 0, sizeof(struct hotwire_video_info));
	info.width = hotwire_frame->width;
	info.height = hotwire_frame->height;

	frame.info = info;

	struct obs_core_video *video = &obs->video;
	send_hotwire_frame(video->video, &frame);
}
...
void *obs_graphics_thread(void *param)
{
	...
	while (!video_output_stopped(obs->video.video)) {
		...
		profile_start("hotwire frame");
		hotwire_frame = find_hotwire_frame();
		output_hotwire_frame(hotwire_frame);
		obs_source_frame_destroy(hotwire_frame);
		profile_end("hotwire frame");
